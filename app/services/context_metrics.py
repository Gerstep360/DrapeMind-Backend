...
async def context_metrics(client, messages, chat_id=None, config=None, role="main"):
    ...
    if settings.AI_CONTEXT_TOKEN_METRICS:
        root = config.AI_BASE_URL.rstrip("/").removesuffix("/v1")
        for key, value in parts.items():
            cache_key = (root, config.AI_MODEL, key, value) if key in {"system", "tools"} else None
    ...
    try:
        ...
        entities = None
    record = {"chat": chat_id, "role": role, "sections": counts, "prompt_characters": sum(x["chars"] for x in counts.values()),
