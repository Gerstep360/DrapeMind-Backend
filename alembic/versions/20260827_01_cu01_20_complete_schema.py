"""DrapeMind Atelier - Migración Inicial Completa (CU-01 a CU-20 & Asistente IA)

Esta migración inicial crea desde cero todo el esquema de la base de datos PostgreSQL:
- Extensiones necesarias (pgcrypto, citext).
- Tipos ENUM del sistema.
- Función set_updated_at().
- Tablas completas: usuarios, ciudades, sucursales, personal_sucursal, direcciones,
  categorias, productos, variantes_producto, stock_sucursal, movimientos_inventario,
  carritos, items_carrito, reservas, items_reserva, pedidos, items_pedido,
  pagos, favoritos, ai_sesiones, ai_interacciones, ai_recomendaciones.
- Índices, restricciones de integridad, triggers de updated_at y vistas analíticas.
- Registro inicial de ciudad y sucursal central por defecto.

Revision ID: 20260827_01
Revises: None
Create Date: 2026-08-27 12:00:00.000000
"""

from alembic import op

# Identificadores de revisión de Alembic
revision = "20260827_01"
down_revision = None
branch_labels = None
depends_on = None


def upgrade() -> None:
    # 1. EXTENSIONES
    op.execute("CREATE EXTENSION IF NOT EXISTS pgcrypto;")
    op.execute("CREATE EXTENSION IF NOT EXISTS citext;")

    # 2. ENUMS
    op.execute("""
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'rol_usuario') THEN
            CREATE TYPE rol_usuario AS ENUM ('CLIENTE', 'ADMIN', 'VENDEDOR', 'ENCARGADO', 'CAJERO');
        ELSE
            ALTER TYPE rol_usuario ADD VALUE IF NOT EXISTS 'ENCARGADO';
            ALTER TYPE rol_usuario ADD VALUE IF NOT EXISTS 'CAJERO';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_usuario') THEN
            CREATE TYPE estado_usuario AS ENUM ('ACTIVO', 'BLOQUEADO', 'INACTIVO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'genero_objetivo') THEN
            CREATE TYPE genero_objetivo AS ENUM ('HOMBRE', 'MUJER', 'UNISEX', 'OTRO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_carrito') THEN
            CREATE TYPE estado_carrito AS ENUM ('ACTIVO', 'CONVERTIDO', 'ABANDONADO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_movimiento_inventario') THEN
            CREATE TYPE tipo_movimiento_inventario AS ENUM ('ENTRADA', 'VENTA', 'RESERVA', 'LIBERACION_RESERVA', 'DEVOLUCION', 'AJUSTE');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_reserva') THEN
            CREATE TYPE estado_reserva AS ENUM ('PENDIENTE', 'CONFIRMADA', 'EN_PREPARACION', 'LISTA', 'RETIRADA', 'VENCIDA', 'CANCELADA', 'CONVERTIDA');
        ELSE
            ALTER TYPE estado_reserva ADD VALUE IF NOT EXISTS 'EN_PREPARACION';
            ALTER TYPE estado_reserva ADD VALUE IF NOT EXISTS 'LISTA';
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_pedido') THEN
            CREATE TYPE estado_pedido AS ENUM ('PENDIENTE_PAGO', 'PAGADO', 'PREPARANDO', 'LISTO', 'ENVIADO', 'ENTREGADO', 'CANCELADO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'canal_pedido') THEN
            CREATE TYPE canal_pedido AS ENUM ('MOBILE', 'WEB', 'TIENDA');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_entrega') THEN
            CREATE TYPE tipo_entrega AS ENUM ('DELIVERY', 'RECOJO', 'TIENDA');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'metodo_pago') THEN
            CREATE TYPE metodo_pago AS ENUM ('QR', 'TARJETA', 'EFECTIVO', 'TRANSFERENCIA');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_pago') THEN
            CREATE TYPE estado_pago AS ENUM ('PENDIENTE', 'PROCESANDO', 'APROBADO', 'RECHAZADO', 'REEMBOLSADO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_ai_sesion') THEN
            CREATE TYPE estado_ai_sesion AS ENUM ('ACTIVA', 'CERRADA', 'EXPIRADA');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_ai_interaccion') THEN
            CREATE TYPE tipo_ai_interaccion AS ENUM ('CHAT', 'PRODUCT_SEARCH', 'GENERATE_OUTFIT', 'COMPLETE_OUTFIT', 'STYLE_CHECK', 'VALUE_CHECK');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_ai_interaccion') THEN
            CREATE TYPE estado_ai_interaccion AS ENUM ('OK', 'ERROR', 'CANCELADO');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_ai_recomendacion') THEN
            CREATE TYPE tipo_ai_recomendacion AS ENUM ('OUTFIT', 'REEMPLAZO_ESTILO', 'REEMPLAZO_AHORRO', 'REEMPLAZO_VALOR', 'COMPLETAR_OUTFIT');
        END IF;

        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'rol_prenda_outfit') THEN
            CREATE TYPE rol_prenda_outfit AS ENUM ('TOP', 'BOTTOM', 'SHOES', 'OUTERWEAR', 'ACCESSORY', 'OTHER');
        END IF;
    END $$;
    """)

    # 3. FUNCIÓN UPDATED_AT
    op.execute("""
    CREATE OR REPLACE FUNCTION set_updated_at()
    RETURNS TRIGGER AS $$
    BEGIN
        NEW.updated_at = NOW();
        RETURN NEW;
    END;
    $$ LANGUAGE plpgsql;
    """)

    # 4. TABLAS PRINCIPALES
    # 4.1 USUARIOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS usuarios (
        id              BIGSERIAL PRIMARY KEY,
        nombre          VARCHAR(120) NOT NULL,
        email           CITEXT NOT NULL UNIQUE,
        password_hash   VARCHAR(255) NOT NULL,
        telefono        VARCHAR(30),
        rol             rol_usuario NOT NULL DEFAULT 'CLIENTE',
        estado          estado_usuario NOT NULL DEFAULT 'ACTIVO',
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
    """)

    # 4.2 CIUDADES
    op.execute("""
    CREATE TABLE IF NOT EXISTS ciudades (
        id              BIGSERIAL PRIMARY KEY,
        nombre          VARCHAR(100) NOT NULL,
        departamento    VARCHAR(100) NOT NULL,
        activo          BOOLEAN NOT NULL DEFAULT TRUE,
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT uq_ciudad_nombre_departamento UNIQUE(nombre, departamento)
    );
    """)

    # 4.3 SUCURSALES
    op.execute("""
    CREATE TABLE IF NOT EXISTS sucursales (
        id              BIGSERIAL PRIMARY KEY,
        ciudad_id       BIGINT NOT NULL REFERENCES ciudades(id) ON DELETE RESTRICT,
        codigo          VARCHAR(30) NOT NULL UNIQUE,
        nombre          VARCHAR(120) NOT NULL,
        direccion       VARCHAR(250) NOT NULL,
        telefono        VARCHAR(30),
        latitud         NUMERIC(9,6),
        longitud        NUMERIC(9,6),
        activo          BOOLEAN NOT NULL DEFAULT TRUE,
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
        CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
    );
    """)

    # 4.4 PERSONAL_SUCURSAL
    op.execute("""
    CREATE TABLE IF NOT EXISTS personal_sucursal (
        id              BIGSERIAL PRIMARY KEY,
        usuario_id      BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
        sucursal_id     BIGINT NOT NULL REFERENCES sucursales(id) ON DELETE CASCADE,
        activo          BOOLEAN NOT NULL DEFAULT TRUE,
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT uq_personal_usuario_sucursal UNIQUE(usuario_id, sucursal_id)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_personal_sucursal_usuario ON personal_sucursal(usuario_id, activo);")

    # 4.5 DIRECCIONES
    op.execute("""
    CREATE TABLE IF NOT EXISTS direcciones (
        id                  BIGSERIAL PRIMARY KEY,
        usuario_id          BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
        alias               VARCHAR(50) NOT NULL DEFAULT 'Casa',
        departamento        VARCHAR(80) NOT NULL,
        ciudad              VARCHAR(80) NOT NULL,
        zona                VARCHAR(100),
        direccion           VARCHAR(250) NOT NULL,
        referencia          VARCHAR(250),
        latitud             NUMERIC(9,6),
        longitud            NUMERIC(9,6),
        telefono_contacto   VARCHAR(30),
        es_principal        BOOLEAN NOT NULL DEFAULT FALSE,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (latitud IS NULL OR latitud BETWEEN -90 AND 90),
        CHECK (longitud IS NULL OR longitud BETWEEN -180 AND 180)
    );
    """)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_direccion_principal_usuario ON direcciones(usuario_id) WHERE es_principal = TRUE;")

    # 4.6 CATEGORIAS
    op.execute("""
    CREATE TABLE IF NOT EXISTS categorias (
        id              BIGSERIAL PRIMARY KEY,
        nombre          VARCHAR(80) NOT NULL,
        slug            VARCHAR(80) NOT NULL UNIQUE,
        descripcion     VARCHAR(250),
        parent_id       BIGINT REFERENCES categorias(id) ON DELETE SET NULL,
        activo          BOOLEAN NOT NULL DEFAULT TRUE,
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT uq_categorias_parent_nombre UNIQUE(parent_id, nombre)
    );
    """)

    # 4.7 PRODUCTOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS productos (
        id                  BIGSERIAL PRIMARY KEY,
        categoria_id        BIGINT NOT NULL REFERENCES categorias(id) ON DELETE RESTRICT,
        nombre              VARCHAR(150) NOT NULL,
        descripcion         TEXT,
        marca               VARCHAR(100),
        material            VARCHAR(150),
        precio              NUMERIC(10,2) NOT NULL,
        costo_referencia    NUMERIC(10,2),
        calidad_nivel       SMALLINT NOT NULL DEFAULT 3,
        genero_objetivo     genero_objetivo NOT NULL DEFAULT 'UNISEX',
        descripcion_ai      TEXT,
        tags_ai             TEXT[],
        imagenes            JSONB NOT NULL DEFAULT '[]'::jsonb,
        activo              BOOLEAN NOT NULL DEFAULT TRUE,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (precio >= 0),
        CHECK (costo_referencia IS NULL OR costo_referencia >= 0),
        CHECK (calidad_nivel BETWEEN 1 AND 5),
        CHECK (jsonb_typeof(imagenes) = 'array')
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_productos_categoria_activo ON productos(categoria_id, activo);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_productos_precio ON productos(precio);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_productos_tags_ai ON productos USING GIN(tags_ai);")

    # 4.8 VARIANTES_PRODUCTO
    op.execute("""
    CREATE TABLE IF NOT EXISTS variantes_producto (
        id                  BIGSERIAL PRIMARY KEY,
        producto_id         BIGINT NOT NULL REFERENCES productos(id) ON DELETE RESTRICT,
        sku                 VARCHAR(80) NOT NULL UNIQUE,
        color               VARCHAR(60) NOT NULL,
        codigo_color        VARCHAR(15),
        talla               VARCHAR(20) NOT NULL,
        stock_total         INTEGER NOT NULL DEFAULT 0,
        stock_reservado     INTEGER NOT NULL DEFAULT 0,
        codigo_barras       VARCHAR(100) UNIQUE,
        imagen              VARCHAR(500),
        activo              BOOLEAN NOT NULL DEFAULT TRUE,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (stock_total >= 0),
        CHECK (stock_reservado >= 0),
        CHECK (stock_reservado <= stock_total),
        CONSTRAINT uq_variantes_producto_color_talla UNIQUE(producto_id, color, talla)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_variantes_producto ON variantes_producto(producto_id, activo);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_variantes_stock ON variantes_producto(producto_id, stock_total, stock_reservado);")

    # 4.9 STOCK_SUCURSAL
    op.execute("""
    CREATE TABLE IF NOT EXISTS stock_sucursal (
        id                  BIGSERIAL PRIMARY KEY,
        sucursal_id         BIGINT NOT NULL REFERENCES sucursales(id) ON DELETE CASCADE,
        variante_id         BIGINT NOT NULL REFERENCES variantes_producto(id) ON DELETE RESTRICT,
        stock_total         INTEGER NOT NULL DEFAULT 0,
        stock_reservado     INTEGER NOT NULL DEFAULT 0,
        stock_minimo        INTEGER NOT NULL DEFAULT 0,
        activo              BOOLEAN NOT NULL DEFAULT TRUE,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT uq_stock_sucursal_variante UNIQUE(sucursal_id, variante_id),
        CHECK (stock_total >= 0),
        CHECK (stock_reservado >= 0),
        CHECK (stock_minimo >= 0),
        CHECK (stock_reservado <= stock_total)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_stock_sucursal_disponible ON stock_sucursal(sucursal_id, activo, variante_id);")

    # 4.10 MOVIMIENTOS_INVENTARIO
    op.execute("""
    CREATE TABLE IF NOT EXISTS movimientos_inventario (
        id                          BIGSERIAL PRIMARY KEY,
        variante_id                 BIGINT NOT NULL REFERENCES variantes_producto(id) ON DELETE RESTRICT,
        sucursal_id                 BIGINT REFERENCES sucursales(id) ON DELETE SET NULL,
        tipo                        tipo_movimiento_inventario NOT NULL,
        cantidad                    INTEGER NOT NULL,
        stock_total_anterior        INTEGER NOT NULL,
        stock_total_nuevo           INTEGER NOT NULL,
        stock_reservado_anterior    INTEGER NOT NULL,
        stock_reservado_nuevo       INTEGER NOT NULL,
        referencia_tipo             VARCHAR(30),
        referencia_id               BIGINT,
        usuario_id                  BIGINT REFERENCES usuarios(id) ON DELETE SET NULL,
        observacion                 VARCHAR(300),
        created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (cantidad <> 0),
        CHECK (stock_total_anterior >= 0),
        CHECK (stock_total_nuevo >= 0),
        CHECK (stock_reservado_anterior >= 0),
        CHECK (stock_reservado_nuevo >= 0),
        CHECK (stock_reservado_anterior <= stock_total_anterior),
        CHECK (stock_reservado_nuevo <= stock_total_nuevo)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_movimientos_variante_fecha ON movimientos_inventario(variante_id, created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_movimientos_referencia ON movimientos_inventario(referencia_tipo, referencia_id);")

    # 4.11 CARRITOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS carritos (
        id              BIGSERIAL PRIMARY KEY,
        usuario_id      BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
        estado          estado_carrito NOT NULL DEFAULT 'ACTIVO',
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );
    """)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_carrito_activo_usuario ON carritos(usuario_id) WHERE estado = 'ACTIVO';")
    op.execute("CREATE INDEX IF NOT EXISTS ix_carritos_usuario_estado ON carritos(usuario_id, estado);")

    # 4.12 ITEMS_CARRITO
    op.execute("""
    CREATE TABLE IF NOT EXISTS items_carrito (
        id                  BIGSERIAL PRIMARY KEY,
        carrito_id          BIGINT NOT NULL REFERENCES carritos(id) ON DELETE CASCADE,
        variante_id         BIGINT NOT NULL REFERENCES variantes_producto(id) ON DELETE RESTRICT,
        cantidad            INTEGER NOT NULL DEFAULT 1,
        precio_referencia   NUMERIC(10,2) NOT NULL,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (cantidad > 0),
        CHECK (precio_referencia >= 0),
        CONSTRAINT uq_items_carrito_variante UNIQUE(carrito_id, variante_id)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_items_carrito_carrito ON items_carrito(carrito_id);")

    # 4.13 RESERVAS
    op.execute("""
    CREATE TABLE IF NOT EXISTS reservas (
        id                  BIGSERIAL PRIMARY KEY,
        codigo_publico      UUID NOT NULL UNIQUE DEFAULT gen_random_uuid(),
        usuario_id          BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
        sucursal_id         BIGINT REFERENCES sucursales(id) ON DELETE RESTRICT,
        estado              estado_reserva NOT NULL DEFAULT 'PENDIENTE',
        fecha_reserva       TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        vence_at            TIMESTAMPTZ NOT NULL,
        qr_token            UUID NOT NULL UNIQUE DEFAULT gen_random_uuid(),
        observacion         VARCHAR(300),
        preparado_por_id    BIGINT REFERENCES usuarios(id) ON DELETE SET NULL,
        preparado_at        TIMESTAMPTZ,
        atendido_por_id     BIGINT REFERENCES usuarios(id) ON DELETE SET NULL,
        atendido_at         TIMESTAMPTZ,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (vence_at > fecha_reserva)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_reservas_usuario_estado ON reservas(usuario_id, estado);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_reservas_vence_at ON reservas(vence_at);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_reservas_sucursal_estado ON reservas(sucursal_id, estado, vence_at);")

    # 4.14 ITEMS_RESERVA
    op.execute("""
    CREATE TABLE IF NOT EXISTS items_reserva (
        id                  BIGSERIAL PRIMARY KEY,
        reserva_id          BIGINT NOT NULL REFERENCES reservas(id) ON DELETE CASCADE,
        variante_id         BIGINT NOT NULL REFERENCES variantes_producto(id) ON DELETE RESTRICT,
        cantidad            INTEGER NOT NULL DEFAULT 1,
        precio_referencia   NUMERIC(10,2) NOT NULL,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (cantidad > 0),
        CHECK (precio_referencia >= 0),
        CONSTRAINT uq_items_reserva_variante UNIQUE(reserva_id, variante_id)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_items_reserva_reserva ON items_reserva(reserva_id);")

    # 4.15 PEDIDOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS pedidos (
        id                          BIGSERIAL PRIMARY KEY,
        codigo_publico              UUID NOT NULL UNIQUE DEFAULT gen_random_uuid(),
        usuario_id                  BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE RESTRICT,
        reserva_id                  BIGINT UNIQUE REFERENCES reservas(id) ON DELETE SET NULL,
        sucursal_id                 BIGINT REFERENCES sucursales(id) ON DELETE SET NULL,
        estado                      estado_pedido NOT NULL DEFAULT 'PENDIENTE_PAGO',
        canal                       canal_pedido NOT NULL,
        tipo_entrega                tipo_entrega NOT NULL,
        subtotal                    NUMERIC(10,2) NOT NULL DEFAULT 0,
        descuento                   NUMERIC(10,2) NOT NULL DEFAULT 0,
        costo_envio                 NUMERIC(10,2) NOT NULL DEFAULT 0,
        total                       NUMERIC(10,2) NOT NULL DEFAULT 0,
        direccion_entrega_snapshot  JSONB,
        observacion                 VARCHAR(300),
        created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        updated_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        paid_at                     TIMESTAMPTZ,
        completed_at                TIMESTAMPTZ,
        cancelled_at                TIMESTAMPTZ,
        CHECK (subtotal >= 0),
        CHECK (descuento >= 0),
        CHECK (costo_envio >= 0),
        CHECK (total >= 0),
        CHECK (descuento <= subtotal),
        CHECK (total = subtotal - descuento + costo_envio),
        CHECK (
            direccion_entrega_snapshot IS NULL
            OR jsonb_typeof(direccion_entrega_snapshot) IN ('object', 'null')
        )
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_pedidos_usuario_estado ON pedidos(usuario_id, estado);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_pedidos_fecha ON pedidos(created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_pedidos_estado_fecha ON pedidos(estado, created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_pedidos_sucursal_estado ON pedidos(sucursal_id, estado);")

    # 4.16 ITEMS_PEDIDO
    op.execute("""
    CREATE TABLE IF NOT EXISTS items_pedido (
        id                  BIGSERIAL PRIMARY KEY,
        pedido_id           BIGINT NOT NULL REFERENCES pedidos(id) ON DELETE RESTRICT,
        producto_id         BIGINT REFERENCES productos(id) ON DELETE SET NULL,
        variante_id         BIGINT REFERENCES variantes_producto(id) ON DELETE SET NULL,
        nombre_snapshot     VARCHAR(150) NOT NULL,
        sku_snapshot        VARCHAR(80) NOT NULL,
        color_snapshot      VARCHAR(60) NOT NULL,
        talla_snapshot      VARCHAR(20) NOT NULL,
        cantidad            INTEGER NOT NULL,
        precio_unitario     NUMERIC(10,2) NOT NULL,
        descuento           NUMERIC(10,2) NOT NULL DEFAULT 0,
        subtotal            NUMERIC(10,2) NOT NULL,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (cantidad > 0),
        CHECK (precio_unitario >= 0),
        CHECK (descuento >= 0),
        CHECK (subtotal >= 0),
        CHECK (descuento <= precio_unitario * cantidad),
        CHECK (subtotal = (precio_unitario * cantidad) - descuento)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_items_pedido_pedido ON items_pedido(pedido_id);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_items_pedido_producto ON items_pedido(producto_id);")

    # 4.17 PAGOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS pagos (
        id                  BIGSERIAL PRIMARY KEY,
        pedido_id           BIGINT NOT NULL REFERENCES pedidos(id) ON DELETE RESTRICT,
        metodo              metodo_pago NOT NULL,
        proveedor           VARCHAR(80),
        monto               NUMERIC(10,2) NOT NULL,
        moneda              CHAR(3) NOT NULL DEFAULT 'BOB',
        estado              estado_pago NOT NULL DEFAULT 'PENDIENTE',
        referencia_externa  VARCHAR(200),
        idempotency_key     VARCHAR(100),
        qr_payload          TEXT,
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        paid_at             TIMESTAMPTZ,
        CHECK (monto > 0)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_pagos_pedido_estado ON pagos(pedido_id, estado);")
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_pago_referencia_externa ON pagos(referencia_externa) WHERE referencia_externa IS NOT NULL;")
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_pagos_idempotency_key ON pagos(idempotency_key) WHERE idempotency_key IS NOT NULL;")

    # 4.18 FAVORITOS
    op.execute("""
    CREATE TABLE IF NOT EXISTS favoritos (
        id              BIGSERIAL PRIMARY KEY,
        usuario_id      BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
        producto_id     BIGINT NOT NULL REFERENCES productos(id) ON DELETE CASCADE,
        created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CONSTRAINT uq_favoritos_usuario_producto UNIQUE(usuario_id, producto_id)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_favoritos_usuario ON favoritos(usuario_id);")

    # 4.19 AI_SESIONES
    op.execute("""
    CREATE TABLE IF NOT EXISTS ai_sesiones (
        id                  BIGSERIAL PRIMARY KEY,
        usuario_id          BIGINT NOT NULL REFERENCES usuarios(id) ON DELETE CASCADE,
        estado              estado_ai_sesion NOT NULL DEFAULT 'ACTIVA',
        resumen_contexto    TEXT,
        started_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        last_activity_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        closed_at           TIMESTAMPTZ
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_sesiones_usuario_estado ON ai_sesiones(usuario_id, estado);")

    # 4.20 AI_INTERACCIONES
    op.execute("""
    CREATE TABLE IF NOT EXISTS ai_interacciones (
        id                  BIGSERIAL PRIMARY KEY,
        sesion_id           BIGINT NOT NULL REFERENCES ai_sesiones(id) ON DELETE CASCADE,
        tipo                tipo_ai_interaccion NOT NULL,
        intent              VARCHAR(40),
        mensaje_usuario     TEXT,
        respuesta           TEXT,
        tool_principal      VARCHAR(60),
        duracion_ms         INTEGER,
        tokens_entrada      INTEGER,
        tokens_salida       INTEGER,
        modelo              VARCHAR(80) NOT NULL DEFAULT 'gemma-4-e2b-it',
        estado              estado_ai_interaccion NOT NULL DEFAULT 'OK',
        created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (duracion_ms IS NULL OR duracion_ms >= 0),
        CHECK (tokens_entrada IS NULL OR tokens_entrada >= 0),
        CHECK (tokens_salida IS NULL OR tokens_salida >= 0)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_interacciones_sesion_fecha ON ai_interacciones(sesion_id, created_at DESC);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_interacciones_tipo_fecha ON ai_interacciones(tipo, created_at DESC);")

    # 4.21 AI_RECOMENDACIONES
    op.execute("""
    CREATE TABLE IF NOT EXISTS ai_recomendaciones (
        id                          BIGSERIAL PRIMARY KEY,
        interaccion_id              BIGINT NOT NULL REFERENCES ai_interacciones(id) ON DELETE CASCADE,
        grupo_uuid                  UUID NOT NULL DEFAULT gen_random_uuid(),
        tipo                        tipo_ai_recomendacion NOT NULL,
        rol                         rol_prenda_outfit,
        producto_origen_id          BIGINT REFERENCES productos(id) ON DELETE SET NULL,
        variante_origen_id          BIGINT REFERENCES variantes_producto(id) ON DELETE SET NULL,
        producto_recomendado_id     BIGINT NOT NULL REFERENCES productos(id) ON DELETE RESTRICT,
        variante_recomendada_id     BIGINT REFERENCES variantes_producto(id) ON DELETE SET NULL,
        score                       NUMERIC(5,4),
        ahorro                      NUMERIC(10,2),
        motivo_corto                VARCHAR(300),
        aceptada                    BOOLEAN,
        aplicada                    BOOLEAN NOT NULL DEFAULT FALSE,
        created_at                  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        applied_at                  TIMESTAMPTZ,
        CHECK (score IS NULL OR score BETWEEN 0 AND 1),
        CHECK (ahorro IS NULL OR ahorro >= 0),
        CHECK (aplicada = FALSE OR aceptada = TRUE)
    );
    """)
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_recomendaciones_interaccion ON ai_recomendaciones(interaccion_id);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_recomendaciones_grupo ON ai_recomendaciones(grupo_uuid);")
    op.execute("CREATE INDEX IF NOT EXISTS ix_ai_recomendaciones_producto ON ai_recomendaciones(producto_recomendado_id);")

    # 5. TRIGGERS UPDATED_AT
    op.execute("""
    DO $$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_usuarios_updated_at') THEN
            CREATE TRIGGER trg_usuarios_updated_at BEFORE UPDATE ON usuarios FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_ciudades_updated_at') THEN
            CREATE TRIGGER trg_ciudades_updated_at BEFORE UPDATE ON ciudades FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_sucursales_updated_at') THEN
            CREATE TRIGGER trg_sucursales_updated_at BEFORE UPDATE ON sucursales FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_direcciones_updated_at') THEN
            CREATE TRIGGER trg_direcciones_updated_at BEFORE UPDATE ON direcciones FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_categorias_updated_at') THEN
            CREATE TRIGGER trg_categorias_updated_at BEFORE UPDATE ON categorias FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_productos_updated_at') THEN
            CREATE TRIGGER trg_productos_updated_at BEFORE UPDATE ON productos FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_variantes_updated_at') THEN
            CREATE TRIGGER trg_variantes_updated_at BEFORE UPDATE ON variantes_producto FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_stock_sucursal_updated_at') THEN
            CREATE TRIGGER trg_stock_sucursal_updated_at BEFORE UPDATE ON stock_sucursal FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_carritos_updated_at') THEN
            CREATE TRIGGER trg_carritos_updated_at BEFORE UPDATE ON carritos FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_items_carrito_updated_at') THEN
            CREATE TRIGGER trg_items_carrito_updated_at BEFORE UPDATE ON items_carrito FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_reservas_updated_at') THEN
            CREATE TRIGGER trg_reservas_updated_at BEFORE UPDATE ON reservas FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_pedidos_updated_at') THEN
            CREATE TRIGGER trg_pedidos_updated_at BEFORE UPDATE ON pedidos FOR EACH ROW EXECUTE FUNCTION set_updated_at();
        END IF;
    END $$;
    """)

    # 6. VISTAS SQL
    op.execute("""
    CREATE OR REPLACE VIEW vw_stock_disponible AS
    SELECT
        vp.id AS variante_id,
        vp.producto_id,
        vp.sku,
        vp.color,
        vp.talla,
        vp.stock_total,
        vp.stock_reservado,
        (vp.stock_total - vp.stock_reservado) AS stock_disponible,
        vp.activo
    FROM variantes_producto vp;
    """)

    op.execute("""
    CREATE OR REPLACE VIEW vw_historial_ventas AS
    SELECT
        p.id AS pedido_id,
        p.codigo_publico,
        p.usuario_id,
        u.nombre AS cliente,
        p.canal,
        p.tipo_entrega,
        p.subtotal,
        p.descuento,
        p.costo_envio,
        p.total,
        p.created_at,
        p.completed_at
    FROM pedidos p
    JOIN usuarios u ON u.id = p.usuario_id
    WHERE p.estado = 'ENTREGADO';
    """)

    op.execute("""
    CREATE OR REPLACE VIEW vw_resumen_ai AS
    SELECT
        ai.tipo,
        COUNT(*) AS total_interacciones,
        AVG(ai.duracion_ms)::NUMERIC(12,2) AS duracion_promedio_ms,
        SUM(CASE WHEN ai.estado = 'OK' THEN 1 ELSE 0 END) AS exitosas,
        SUM(CASE WHEN ai.estado = 'ERROR' THEN 1 ELSE 0 END) AS errores
    FROM ai_interacciones ai
    GROUP BY ai.tipo;
    """)

    # 7. REGISTRO BASE: CIUDAD Y SUCURSAL CENTRAL POR DEFECTO
    op.execute("""
    INSERT INTO ciudades(nombre, departamento) VALUES ('Santa Cruz de la Sierra', 'Santa Cruz')
    ON CONFLICT (nombre, departamento) DO NOTHING;
    """)
    op.execute("""
    INSERT INTO sucursales(ciudad_id, codigo, nombre, direccion, telefono)
    SELECT id, 'SCZ-CENTRAL', 'DrapeMind Central', 'Av. San Martín, Equipetrol', '70000000'
    FROM ciudades WHERE nombre='Santa Cruz de la Sierra' AND departamento='Santa Cruz'
    ON CONFLICT (codigo) DO NOTHING;
    """)


def downgrade() -> None:
    # VISTAS
    op.execute("DROP VIEW IF EXISTS vw_resumen_ai CASCADE;")
    op.execute("DROP VIEW IF EXISTS vw_historial_ventas CASCADE;")
    op.execute("DROP VIEW IF EXISTS vw_stock_disponible CASCADE;")

    # TABLAS
    op.execute("DROP TABLE IF EXISTS ai_recomendaciones CASCADE;")
    op.execute("DROP TABLE IF EXISTS ai_interacciones CASCADE;")
    op.execute("DROP TABLE IF EXISTS ai_sesiones CASCADE;")
    op.execute("DROP TABLE IF EXISTS favoritos CASCADE;")
    op.execute("DROP TABLE IF EXISTS pagos CASCADE;")
    op.execute("DROP TABLE IF EXISTS items_pedido CASCADE;")
    op.execute("DROP TABLE IF EXISTS pedidos CASCADE;")
    op.execute("DROP TABLE IF EXISTS items_reserva CASCADE;")
    op.execute("DROP TABLE IF EXISTS reservas CASCADE;")
    op.execute("DROP TABLE IF EXISTS items_carrito CASCADE;")
    op.execute("DROP TABLE IF EXISTS carritos CASCADE;")
    op.execute("DROP TABLE IF EXISTS movimientos_inventario CASCADE;")
    op.execute("DROP TABLE IF EXISTS stock_sucursal CASCADE;")
    op.execute("DROP TABLE IF EXISTS variantes_producto CASCADE;")
    op.execute("DROP TABLE IF EXISTS productos CASCADE;")
    op.execute("DROP TABLE IF EXISTS categorias CASCADE;")
    op.execute("DROP TABLE IF EXISTS direcciones CASCADE;")
    op.execute("DROP TABLE IF EXISTS personal_sucursal CASCADE;")
    op.execute("DROP TABLE IF EXISTS sucursales CASCADE;")
    op.execute("DROP TABLE IF EXISTS ciudades CASCADE;")
    op.execute("DROP TABLE IF EXISTS usuarios CASCADE;")

    # FUNCIÓN
    op.execute("DROP FUNCTION IF EXISTS set_updated_at CASCADE;")

    # ENUMS
    op.execute("""
    DO $$
    BEGIN
        DROP TYPE IF EXISTS rol_prenda_outfit CASCADE;
        DROP TYPE IF EXISTS tipo_ai_recomendacion CASCADE;
        DROP TYPE IF EXISTS estado_ai_interaccion CASCADE;
        DROP TYPE IF EXISTS tipo_ai_interaccion CASCADE;
        DROP TYPE IF EXISTS estado_ai_sesion CASCADE;
        DROP TYPE IF EXISTS estado_pago CASCADE;
        DROP TYPE IF EXISTS metodo_pago CASCADE;
        DROP TYPE IF EXISTS tipo_entrega CASCADE;
        DROP TYPE IF EXISTS canal_pedido CASCADE;
        DROP TYPE IF EXISTS estado_pedido CASCADE;
        DROP TYPE IF EXISTS estado_reserva CASCADE;
        DROP TYPE IF EXISTS tipo_movimiento_inventario CASCADE;
        DROP TYPE IF EXISTS estado_carrito CASCADE;
        DROP TYPE IF EXISTS genero_objetivo CASCADE;
        DROP TYPE IF EXISTS estado_usuario CASCADE;
        DROP TYPE IF EXISTS rol_usuario CASCADE;
    END $$;
    """)
